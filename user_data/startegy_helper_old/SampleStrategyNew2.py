...
class SampleStrategyNew2(IStrategy):
    TGP_dict={}
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if not (self.TGP_dict.get(metadata['pair']) and not isinstance(self.TGP_dict.get(metadata['pair']), Trailing_Gain_Util)):
        ...
        if self.dp._exchange is none:
            dtemp=dataframe;
            for i range (0,len(dtemp.index)):
                if i==0:
                    pass;
                else:
                    if dataframe.loc[i]['high']<dtemp.loc[i-1]['high']:
